...
**Answer** I tried to read the file using `read_csv` function as follows:
...
It solved the ingestion issue. The file was read around 5 mins. However, the file is still quite large, and the memory usage is high.
...
Finally, to measure how long it takes to ingest `labevents_filtered.csv.gz`. It takes around 23 seconds to ingest the new file `labevents_filtered.csv.gz`.
...
The ingest+select+filter process takes around 0.05140901 secs. The number of rows in the result tibble is 24855909.

Then, to display the first 10 rows of the result tibble and they match those in Q2.3.
...
The ingest+select+filter process of the Parquet file takes around 0.202384 secs. And the number of rows in the result tibble is 24855909.

Then, to display the first 10 rows of the result tibble and their contents match those in Q2.3, even though the order of the rows may be different.
...
The ingest+convert+select+filter process takes around 38.68542 secs. And the number of rows in the result tibble is 24855909.
