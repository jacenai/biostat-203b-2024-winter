...
rename_with(~dlabitems_tble$name[d_labitems$itemid == .x],
            .cols = -c(subject_id, stay_id))
```
...
# join with icustays_tble to the chartevents table to 
# get the stay_id and intime
left_join(icustays_tble[,c('subject_id', 'stay_id', 'intime', 'outtime')],
          by = "subject_id") |>
# make sure the charted event is within the ICU stay
filter(charttime >= intime) |>
filter(charttime <= outtime) |>
# try to keep the first measurement
group_by(subject_id, stay_id, itemid) |>
# sort by charttime and keep the first measurement
arrange(charttime, by_group = TRUE) |>
slice_head(n = 1) |>
# remove the charttime and intime columns
select(-charttime, -intime, -outtime) |>
# because the form of the `itemid` is in long format,
# we need to convert it to wide format
pivot_wider(names_from = itemid, values_from = valuenum) |>
# use the more meaningful names by renaming the columns
rename_with(~d_items_tble$label[d_items_tble$itemid == .x], 
            .cols = -c(subject_id, stay_id)) |>
print(width = Inf)
...
# admission information
  left_join(admissions_tble, by = "hadm_id", "subject_id") |>
...
mimic_icu_cohort %>%
  group_by(gender) %>%
  ggplot(aes(x = gender, y = los)) +  # Corrected aesthetics specification
  geom_count() +
...
#| eval: true
mimic_icu_cohort |>
  ggplot(aes(y = los, x = age_at_intime)) +
  geom_point() +
  labs(title = "Length of ICU stay vs age at intime",
       x = "Length of ICU stay (days)",
       y = "Age at intime (years)") +
  theme_minimal()
```
...
mimic_icu_cohort_lab_last <- mimic_icu_cohort %>%
  select(los, contains("lab_")) %>%
  drop_na() %>%
  pivot_longer(cols = -los, names_to = "lab_measure", values_to = "value") %>%
  mutate(lab_measure = str_remove(lab_measure, "lab_")) %>%
...
```{r}
mimic_icu_cohort_availability <- mimic_icu_cohort %>%
  select(los, contains("lab_")) %>%
  drop_na() %>%
  pivot_longer(cols = -los, names_to = "lab_measure", values_to = "value") %>%
  mutate(lab_measure = str_remove(lab_measure, "lab_")) %>%
...
# Create boxplot for each vital measure
mimic_icu_cohort_vital <- mimic_icu_cohort %>%
  filter(!is.na(los) & los > 0) %>%
  gather(key = 'vital_measure', value = 'vital_value', 9:13)

# Plotting with line plot and adjusted axis scaling
mimic_icu_cohort_vital %>%
  ggplot(mapping = aes(x = los, y = vital_value, group = vital_measure, color = vital_measure)) +
  geom_bar(stat = "identity") +
  facet_wrap(~vital_measure, scales = "free_y") +  # Adjusted y-axis scaling
  labs(title = "Trends of First Vital Measurements over ICU Stay Length",
       x = "Length of ICU stay (days)",
       y = "Vital measurements") +
  theme_minimal()   
