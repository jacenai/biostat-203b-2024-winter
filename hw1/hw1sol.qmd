...
subject_count=$(zcat < ~/mimic/hosp/admissions.csv.gz | awk -F ',' '{print $1}' | sort -u | wc -l)
echo "Number of unique patients in admissions.csv.gz: $subject_count"
```

```{bash}
#Count the number of unique patients (identified by subject_id) in patients.csv.gz:
subject_count=$(zcat < ~/mimic/hosp/patients.csv.gz | awk -F ',' '{print $1}' | sort -u | wc -l)
echo "Number of unique patients in patients.csv.gz: $subject_count"
```
