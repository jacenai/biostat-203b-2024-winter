...
creatinine (50912), potassium (50971), sodium (50983), chloride (50902), bicarbonate (50882), hematocrit (51221), white blood cell count (51301), and glucose (50931)
```{r}
# 
...
labevents_tble <- tbl(con_bq, "labevents") |>
  ...
  # filter lab items listed in HW3
  filter(itemid %in% c(50912, 50971, 
                       50983, 50902, 
                       50882, 51221, 
                       51301, 50931)) |>
  # keep the last lab measurements before the ICU stay
  # first, inner join with icustays_tble
  left_join(icustays_tble, by = c("hadm_id", "subject_id")) |>
  # then, filter the last lab measurements before the ICU stay
  filter(storetime <= intime) |>
  group_by(subject_id, hadm_id, stay_id, itemid) |> #每个人，每次入院，每次进ICU，每个lab item
